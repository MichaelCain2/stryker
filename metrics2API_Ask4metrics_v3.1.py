...
# NOTES: This function calls the Dynatrace /metrics/{selector} endpoint to get human-friendly labels and units
...
def generate_graph(timestamps, values, metric_name):
    ...
    try:
        ...
        if metric_units.get(metric_name, '') == 'Byte':
            # NOTES: Convert raw bytes to gigabytes (GB) for easier human interpretation
            values = [v / (1024**3) for v in values]  # Convert Bytes to GB
            ax = plt.gca()
            # NOTES: Prevent scientific notation like 1e11 by using plain number formatting
            ax.ticklabel_format(style='plain', axis='y')
            ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
            # NOTES: This hardcodes the Y label to 'GB' when the metric unit is Byte
            plt.ylabel('GB')
        else:
        # NOTES: Use the unit returned by Dynatrace to label the Y-axis for clarity.
