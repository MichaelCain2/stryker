...
    """
    Fetch human-readable hostname from the Entities API. Requires traversing entities API and back to metrics API. Tricky to do and without panda and numpy is difficult at best
    """
...
 
        plt.figure(figsize=(8, 4))  # Adjust chart height
        plt.plot(datetime_timestamps, values, label=metric_name, marker='o', color='blue')
        plt.title(metric_name)
        plt.xlabel("") #Do not need anything here really
        plt.ylabel("") #Do not need anything here really
        plt.grid(True)
        plt.legend()
 
...
    width, height = letter
    margin = 55 #Adjusted from 50 to 55 to try to get a little more seperation
    chart_height = 120  # Reduce chart height
    chart_spacing = 15  # Reduce spacing between charts Adjusted from 20 to 15 to try to get a little less seperation
...
        c.drawString(margin, height - 50, f"Team Name/Management Zone: {management_zone}") #Adjusted from 50 to 35 to try to get a little more seperation between header and hostname
 
    # Add initial header
    c.setFont("Helvetica-Bold", 12)
...
    c.drawString(margin, height - 115, f"Resources/Metrics: {', '.join(metrics)}")
