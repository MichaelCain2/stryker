...
# do not pretend to understand this wizardry completely but spending enough time on the matplotlib site, it did provide enough instruction
...
def create_pdf(grouped_data, management_zone, agg_time, output_pdf):
    ...
    host_start_time = time.time()  # NOTES: Start time for host processing.
    for idx, (host_name, metrics_data) in enumerate(grouped_data.items(), start=1):
        ...
        print_progress(idx, total_hosts, host_start_time, prefix='Processing hosts')  # NOTES: Shows progress for each host processed.
