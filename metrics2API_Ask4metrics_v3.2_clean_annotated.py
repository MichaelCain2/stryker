...
def fetch_metric_metadata(headers, selector):
    ...
    try:
        ...
        meta = response.json()
        unit = meta.get("unit", "")
        display = meta.get("displayName", selector)
        return unit, display
...
# "Average Disk Used Percentage" now uses splitBy("dt.entity.disk"), returning DISK-XXXX entity IDs in the dimension.
    ...
    # NOTES: Prompt the user for one or more metric selectors (Dynatrace metric keys).
...
metrics = {metric.strip(): metric.strip() for metric in metrics_input if metric.strip()}
    # NOTES: Loop through each metric and fetch its unit and display name from Dynatrace.
...
# NOTES: This function sends a request to the Dynatrace Metrics API and returns the result as JSON.
...
# NOTES: This fetches a readable hostname for a given host ID from the Dynatrace Entities API.
...
# NOTES: This function organizes the raw metric data by host and metric name for easier chart generation.
...
# NOTES: Uses matplotlib to draw the graph for each metric using the time series data.
def generate_graph(timestamps, values, metric_name):
    ...
    try:
        ...
        # NOTES: Auto-scale Byte metrics to GB, disable scientific notation for human clarity
        if metric_units.get(metric_name, '') == 'Byte':
            values = [v / (1024**3) for v in values]  # Convert from Bytes to GB
            ax = plt.gca()
            ax.ticklabel_format(style='plain', axis='y')
            ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
            plt.ylabel('GB')
...
# NOTES: This replaces characters in names that can't be used in file names.
...
# NOTES: This uses ReportLab to build the PDF. It creates a title page and adds a graph per metric.
...
# NOTES: This is where the script starts running. It prompts for inputs and kicks off the data fetch and PDF creation.
if __name__ == "__main__":
    overall_start = time.time()

    # NOTES: This is the Dynatrace API base URL, usually ending in /api/v2/metrics/query
    API_URL = input("Enter API URL: ").strip()
    # NOTES: This is the secret token for authentication. It must have rights to read metrics and entities.
    ...
    # NOTES: Query the API for the given metric and store the data for later processing.
    ...
    # NOTES: Reformat the raw API response into a dictionary grouped by host and metric.
    ...
    # NOTES: Begin building the PDF report. One page per host with all requested metrics.
    ...
    # NOTES: Let the user know the PDF is ready and where it was saved.
